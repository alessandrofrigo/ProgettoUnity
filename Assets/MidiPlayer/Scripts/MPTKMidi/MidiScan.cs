                                    ...
                                    if (withMeta)
                                    {
                                        switch (mptkEvent.Meta)
                                        {
                                            case MPTKMeta.SetTempo:
                                                Info.Add(BuildInfoTrack(mptkEvent) + $"SetTempo Tempo:{mptkEvent.Duration} MicrosecondsPerQuarterNote:{mptkEvent.Value}");
                                                break;
                                            ...
                                            case MPTKMeta.TimeSignature:
                                                ...
                                                Info.Add(BuildInfoTrack(mptkEvent) + "TimeSignature " +
                                                    $"Numerator (beats per measure):{mptkEvent.Value} " +
                                                    $"Denominator:{mptkEvent.Duration} - " +
                                                    $"Beat per quarter:{Convert.ToInt32(Mathf.Pow(2, mptkEvent.Duration))}");
                                                break;

                                            case MPTKMeta.KeySignature:
                                                Info.Add(BuildInfoTrack(mptkEvent) + "KeySignature " +
                                                    $"SharpsFlats:{mptkEvent.Value} " +
                                                    $"MajorMinor:{mptkEvent.Duration}");
                                                break;
